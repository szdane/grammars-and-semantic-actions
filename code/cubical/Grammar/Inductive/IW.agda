...
module _ {X : Type ℓX} where
  ...
  map : ∀ (F : Functor X ℓ) {A : X → Grammar ℓA}{B : X → Grammar ℓB}
      → (∀ x → A x ⊢ B x)
      → Container F A ⊢ Container F B
  map F f w (s , child) = s , λ p → lift (f _ _ (lower (child p)))
  ...
  module _ (F : X → Functor X ℓ) where
    ...
    isHomo' : ∀ {A : X → Grammar ℓA}{B : X → Grammar ℓB}
      ...
      → Algebra' B
      → (∀ x → A x ⊢ B x)
      → Type (ℓ-max (ℓ-max ℓX ℓA) (ℓ-max ℓ ℓB))
  ...
  module _ {A : X → Grammar ℓA} where
    ...
    opaque
      ...
      toContainer : (F : Functor X ℓ)
       → ⟦ F ⟧ A ⊢ Container F A
      toContainer F w t = (getShapeF F w t) , lift ∘ (getSubtreeF F w t)

      fromContainer : ∀ (F : Functor X ℓ) → Container F A ⊢ ⟦ F ⟧ A
      fromContainer (k g) _ (s , child) = lift (s .lower)
      fromContainer (Var a) _ (s , child) = child _
      fromContainer (&e B F) _ (s , child) b =
        fromContainer (F b) _ (s b , λ p → lift (lower (child (b , p))))
      fromContainer (⊕e B F) _ ((b , s) , child) = b , fromContainer (F b) _ (s , lift ∘ lower ∘ child ∘ lift)
      fromContainer (⊗e Fl Fr) _ ((splits , sl , sr) , child) =
       splits ,
       fromContainer Fl _ (sl , lift ∘ lower ∘ child ∘ lift ∘ inl) ,
       fromContainer Fr _ (sr , lift ∘ lower ∘ child ∘ lift ∘ inr)

      secFC : ∀ (F : Functor X ℓ) w t → (fromContainer F ∘g toContainer F) w t ≡ t
      secFC (k g) w t = refl
      ...
      retFC1 : ∀ (F : Functor X ℓ) w t →
        (toContainer F ∘g fromContainer F) w t .fst ≡ t .fst
      retFC1 (Var a) w t = refl
      retFC1 (k g) w t = refl
      retFC1 (&e B F) w (s , child) = funExt λ b →
        retFC1 (F b) w (s b , λ p → lift (lower (child (b , p))))
        -- retFC1 (F b) w (s b , (λ p → child (b , p)))
      retFC1 (⊕e B F) w ((b , s) , child) =
        ΣPathP (refl , retFC1 (F b) w (s , lift ∘ lower ∘ child ∘ lift))
      retFC1 (⊗e Fl Fr) w ((splits , sl , sr) , child) =
        ΣPathP (refl , ΣPathP
          ( retFC1 Fl _ (sl , lift ∘ lower ∘ child ∘ lift ∘ inl)
          , retFC1 Fr _ (sr , lift ∘ lower ∘ child ∘ lift ∘ inr)))

      retFC2 : ∀ (F : Functor X ℓ) w t →
        PathP
          (λ i → (p : FP F w (retFC1 F w t i)) →
            Lift (A (F-inX F w (retFC1 F w t i) p .fst) (F-inX F w (retFC1 F w t i) p .snd)))
          ((toContainer F ∘g fromContainer F) w t .snd)
          (t .snd)
      retFC2 (Var a) w _ = refl
      retFC2 (k g) w _ = isContr→isProp isContrΠ⊥* _ _
      retFC2 (⊕e B F) w ((b , s) , child) i (lift p) =
        lift (lower (retFC2 (F b) w (s , lift ∘ lower ∘ child ∘ lift) i p))
      retFC2 (&e B F) w (s , child) i (b , p) =
        lift (lower (retFC2 (F b) w (s b , lift ∘ lower ∘ child ∘ λ a → b , a) i p))
      retFC2 (⊗e Fl Fr) w ((_ , sl , sr) , child) i (lift (inl p)) =
        lift (lower (retFC2 Fl _ (sl , lift ∘ lower ∘ child ∘ lift ∘ inl) i p))
      retFC2 (⊗e Fl Fr) w ((_ , sl , sr) , child) i (lift (inr p)) =
        lift (lower (retFC2 Fr _ (sr , lift ∘ lower ∘ child ∘ lift ∘ inr) i p))

      retFC : ∀ (F : Functor X ℓ) w t → (toContainer F ∘g fromContainer F) w t ≡ t
      retFC F w t = ΣPathP ((retFC1 F w t) , retFC2 F w t)

    F≅Container :
        ∀ (F : Functor X ℓ) → StrongEquivalence (Container F A) (⟦ F ⟧ A)
  ...
  opaque
    ...
    fromContainerNat  : ∀ {A : X → Grammar ℓA}{B : X → Grammar ℓB}(F : Functor X ℓ)
      → (f : ∀ x → A x ⊢ B x)
    ...
    fromContainerNat (⊕e Y F) f i w ((b , s) , child) =
      b , (fromContainerNat (F b) f i w (s , lift ∘ lower ∘ child ∘ lift))
    fromContainerNat (&e Y F) f i w (s , child) b =
      fromContainerNat (F b) f i w (s b , λ p → (lift (lower (child (b , p)))))
    fromContainerNat (⊗e Fl Fr) f i w ((splits , sl , sr) , child) =
      splits
      , fromContainerNat Fl f i _ (sl , lift ∘ lower ∘ child ∘ lift ∘ inl)
      , fromContainerNat Fr f i _ (sr , lift ∘ lower ∘ child ∘ lift ∘ inr)

  module _ (F : X → Functor X ℓ) where
    μ : X → Grammar (ℓ-max ℓ ℓX)
    μ x s = IW (λ ix → FS (F (ix .fst)) (ix .snd))
            (λ ix → FP (F (ix .fst)) (ix .snd))
            (λ ix → F-inX (F (ix .fst)) (ix .snd)) (x , s)

    ContainerToμ : ∀ x → Container (F x) μ ⊢ μ x
    ContainerToμ x w (s , child) = node s (lower ∘ child)
    ...
    initialAlgebra' : Algebra' F μ
    initialAlgebra' = λ x w z → node (z .fst) (lower ∘ (z .snd))
    ...
    module _ {A : X → Grammar ℓA} where
      module _ (α : Algebra' F A) where
        rec' : ∀ x → μ x ⊢ A x
        rec' x w (node s child) = α x w (s , lift ∘ rec' _ _ ∘ child)
        ...
        module _ (f : ∀ x → μ x ⊢ A x) (fIsHomo' : isHomo' F initialAlgebra' α f) where
          ...
          η'' a w (node s subtree) = funExt⁻ (funExt⁻ (fIsHomo' a) w) (s , lift ∘ subtree)
            ∙ λ i → α a w (s , λ p → lift (η'' _ _ (subtree p) i))
