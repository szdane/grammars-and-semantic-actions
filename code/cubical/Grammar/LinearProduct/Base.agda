...
module Grammar.LinearProduct.Base
...
opaque
  ...
  -- (A ⊗ B) w = Σ[ s ∈ Splitting w ] A (s .left) × B (s .right)
  (A ⊗ B) w =
    Σ
      (Σ String (λ w' → Σ String λ w'' → w ≡ (w' ++ w'')))
      λ (w' , w'' , _) → Σ (A w') (λ _ → B w'')
      -- Σ (Splitting w) (λ s → Σ (A (s .left)) λ _ → B (s .right))
...
--   ⊗-intro⊗-intro = refl
  ...
  opaque
    ...
    ⊗-unit-r {A = A} w p = {!!}
      -- subst A
      --   (sym (++-unit-r _) ∙ cong (p .fst .left ++_) (sym (p .snd .snd)) ∙ sym (p .fst .concat))
      --   (p .snd .fst)
