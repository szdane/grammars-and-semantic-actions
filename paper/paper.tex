...
% TODO this should be better formatted but can't use listings with \otimes ^{\ast}
\begin{figure}
\begin{align*}
\data &~A^{*} : L~\where\\
      & \nil : \ltonl {A^{*}} \\
...
$(a^{*} \otimes b) \oplus c$, we show in \cref{fig:nfaderivation} that \stingquote{ab} is
...
\steven{TODO put regex-NFA iso here. Call it Thompson's construction?. Mention
  its mechanized}
...
parametrized by $\epslion$-closed subsets containing $s'$.
...
encode the grammars accepted by Turing machines.
