...
grammars and parses of them internal to \theoryabbc. In each of our examples, we will look at
...
$A^{*}$ to be an inductive\footnote{As we will soon see, our theory defines
...
cannot be captured through linear types alone. Often one wants to define a
\emph{family} of linear types varying over a set, which is a primary motivator
for us to make \theoryabbv a linear-non-linear theory. For the NFA $N$, an indexed inductive
...
\data &~\Trace_{N}~2~(acc : \Bool) : L_{i}~\where\\
      & \mathsf{stop}_{2} : \ltonol {\Trace_{N}~2~\false} \\
