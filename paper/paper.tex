...
\section{Dependent Lambek Calculus by Example}
...
\begin{figure}
  \begin{array}{rrcl}
    ...
    % 
    \textrm{non-linear types} & X , Y , Z & ::= & \mathsf{Nat} \pipe \mathsf{Int} \pipe \mathsf{Empty} \pipe \mathsf{Unit} \pipe \mathsf{SPF}\,X\pipe U \pipe L \pipe !A \pipe \sum_{x:X} Y \pipe \prod_{x:X} Y\pipe M =_A N\\
