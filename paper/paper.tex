...
\begin{figure}
  \begin{mathpar}
    ...
    {}
    {\Gamma; \Delta \vdash \mathsf{fold}(x.b.f)(M, e) : B\,M}
  \end{mathpar}
...
\begin{figure}
  \begin{mathpar}
    \begin{array}{rcl}
      \textrm{non-linear types} X,Y,Z & ::= & \textrm{Empty}\pipe\textrm{Unit}\pipe\textrm{Bool}\pipe\textrm{Nat}
      \pipe \sum_{x:X} Y \pipe \prod_{x:X}Y\pipe \textrm{SPF}\,X \pipe \cdots \max{also eliminators...}\\
      \textrm{linear types} A,B,C & ::= & c \pipe I \pipe A \otimes B \pipe A \lto B \pipe B \tol A
      \pipe \bigoplus_{x:X} A \pipe \bigamp_{x:X} A \pipe \mu e e'\\
      \textrm{non-linear terms} M,N,O & ::= & \cdots\\
      \textrm{linear terms} 
    \end{array}
  \end{mathpar}
