...
To gain intuiton for working in \theoryabbv, we study some examples of
...
$(a \oplus b) \otimes c$ in the context $x : a, y : c$. We think of this term as
...
$\internalize{\stringquote{ac}} = x : A , y : C$. Thus the term
$\inl(x) \otimes y$ from \cref{fig:fingram} details the
manner by which \stringquote{ac} matches the grammar $(a \oplus b) \otimes c$.
...
\max{Is this really true? I don't think it's accurate to say that the ordering restriction on variable usage actually dictates an order in processing variables. And I don't know what you mean by processed without modification.}
...
$A^{*}$ to be an \max{this footnote seems unnecessary}inductive\footnote{As we will soon see, our theory defines
  \emph{indexed} inductive types. We can recover non-indexed inductive types by
  indexing over the unit type.} linear
...
\steven{TODO need to update the proof tree example derivations (\cref{fig:kleenestarderivation,fig:kleeneabstractproof}) to use the presentation of cons that is now in \cref{fig:kleenestarinductive}}
...
$A^{*}$ in a context comprising abstract linear types. In these contexts, it is
...
\steven{I probably can't get away with the inline definition of $f$ in \cref{fig:kleeneabstractproof}}
\max{shouldn't the fold be applied to $x$ and shouldn't $f$ be $\lambda x.\lambda y.\cdots$?}
\begin{figure}
\begin{mathpar}
  \inferrule
  {
...
cannot be captured through linear types\max{not exactly clear what you mean, maybe you mean ``simple'' linear types?} alone. Often one wants to define a
\emph{family} of linear types varying over a set, which is a primary motivator
for us to make \theoryabbv a linear-non-linear theory\max{jargon-y. Be more direct}. For the NFA $N$, an indexed inductive
type of traces through $N$ is given in \cref{fig:nfainductive}.

The type of traces in $N$ is indexed by two pieces of data: a state in $N$ and a
Boolean. For each state $s : N.\mathsf{state}$ and $acc : \Bool$, we define
$\Trace_{N}~s~acc$ which denotes the
type of traces starting at state $s$ and terminating at an arbitrary state. The Boolean flag $acc$ marks if the trace
ends in either an accepting or a rejecting state\max{confusing. One sentence you say arbitrary state and then the next sentence you say it has to match $acc$}. Let us dissect the structure
...
\max{this notation is inconsistent with Agda syntax. You write $acc : \Bool$ but then in the $\mathsf{stop}$ cases you instantiate $\acc$ with a specific boolean}
\max{is the boolean just overcomplicating stuff here?}
\begin{figure}
\begin{align*}
\data &~\Trace_{N}~1~(acc : \Bool) : L_{i}~\where\\
      & \mathsf{stop}_{1} : \ltonl {\Trace_{N}~1~\true}  \\
      & \mathsf{1to2} : \forall (acc : \Bool) \to \ltonl {(b \lto \Trace_{N}~2~acc \lto \Trace_{N}~1~acc)} \\
      & \mathsf{1to3} : \forall (acc : \Bool) \to \ltonl {(\Trace_{N}~3~acc \lto \Trace_{N}~1~acc)} \\
\\
\data &~\Trace_{N}~2~(acc : \Bool) : L_{i}~\where\\
      & \mathsf{stop}_{2} : \ltonl {\Trace_{N}~2~\false} \\
      & \mathsf{2to2} : \forall (acc : \Bool) \to \ltonl {(a \lto \Trace_{N}~2~acc \lto \Trace_{N}~2~acc)} \\
      & \mathsf{2to3} : \forall (acc : \Bool) \to \ltonl {(b \lto \Trace_{N}~3~acc \lto \Trace_{N}~2~acc)}  \\
\\
\data &~\Trace_{N}~3~(acc : \Bool) : L_{i}~\where\\
      & \mathsf{stop}_{3} : \ltonl {\Trace_{N}~3~\true} \\
      & \mathsf{3to1} : \forall (acc : \Bool) \to \ltonl {(a \lto \Trace_{N}~1~acc \lto \Trace_{N}~3~acc)} \\
\end{align*}
...
\begin{figure}
  \begin{mathpar}
    ...
    {\Gamma \vdash f : \ltonl {(A \lto B)} \\
      ...
      \Gamma ; \Delta \vdash p : \applto {f}{a} = \applto {g}{a}}
...
things with this non-linear index, as we have unstricted access to the index
...
In the presence of distributive coproducts (which we probably add a) the following are equivalent:
\begin{itemize}
...
%% 2. Regex, NFA, DFA
\steven{How much is worth explicitly walking through? The examples from earlier
  give motivation, and we are pressed for space}

\paragraph{Regex-NFA/Thompson}
\steven{TODO put regex-NFA iso here. Call it Thompson's constructon?. Mention
  its mechanized}

\paragraph{Determinization of an NFA}
\steven{TODO put determinzation here. Mention its mechanized}
...
\steven{Rename this}
\steven{Compare to persistance modalities from separation logic}
