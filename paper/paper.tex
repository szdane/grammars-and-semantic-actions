...
\newcommand{\epscons}{\varepsilon\mathsf{cons}}
...
grammars over the three character alphabet $\{ a , b , c\}$. Each letter of the
alphabet is a base type in our calculus. For instance, the grammar
...
the context $\internalize{\stringquote{ab}} = x : a , y : b$. Then
\cref{fig:fingram} shows a typing derivation that shows that $\inl(x
\otimes y)$ defines a valid parse tree for the grammar $(a \otimes b)
\oplus c$.
...
\max{I think concrete examples here would be helpful}
...
%
Finally, exchange allows for variables in the context to be reordered,
but the ordering of characters in a string cannot be ignored in
parsing: otherwise we would have that $\inl(x\otimes y)$ is a valid
...
Regular expressions can be encoded as types using base types,
...
function type $\ltonl$. The arrow, $\uparrow$, wrapping these
constructors means that the constructors by themselves do not consume
any resources, and so are \emph{non-linear} values themselves.
...
In \cref{fig:kleenestarderivation}, we demonstrate that
\stringquote{ab} matches $(a^{*} \otimes b) \oplus c$ through repeated
application of the Kleene star constructors. In this proof tree we
expand some syntactic details that are often left implicit. For
instance, every linear context is parameterized by a nonlinear
context, which is syntactically separated by a semicolon. If the
nonlinear context is empty, we may often omit it; however, in this
figure we make this distinction explicit. Further consider the leaves
of the tree. First, the leaves involving the constructors are
nonlinear terms, and thus they occur in the nonlinear empty
context. Then we explicit cast from an nonlinear $\uparrow$-wrapped
type into a linear one. Such casts may often be made implicitly, such
as in $\cref{fig:kleeneabstractproof}$. 
...
We also see here why $\cons$ and $\nil$ should themselves be
non-linear values: we should be able to use them in constructing multiple parse trees.
...
\max{A lot of passive voice and indirect language in these paragraphs ``Often one wants to...'' ``Our method of representing...is''.}
Automata play a critical role in defining parsers, as they often serve
as an operationalized and executable presentation of a formal
grammar. As a final example, consider the representation of $(a^{*}
\otimes b) \oplus c$ as a non-deterministic finite automaton (NFA),
shown in \cref{fig:exampleNFA}.
...
transition from state 1 to state 2 is labeled by the empty string $\varepsilon$, so
...
Next, we introduce one of the most complex and important type linear
type constructor of our development, \emph{indexed inductive linear
...
\item ($\ltonl$ preserves $\oplus$) The function $\ltonl{(\sum_{x:X}\ltonl{(A\,x)} \lto \ltonl{(\bigoplus\limits_{x:X}A\,x)})}$ is an isomorphism
\end{enumerate}
...
\max{what's a terminal object and what's a monomorphism and what's a distributive coproduct}
...
\begin{itemize}
  ...
  \item Given a grammar $B$, all morphisms $B \to A$ are equal\max{is this internal statement? What is a ``morphism''}
  \item $A \& A \cong A$\max{what is $\cong$?}
\end{itemize}
...
that it generates. Two grammars are said to be \emph{weakly equivalent} by
Chomsky\max{you can drop the ``by Chomsky'' at this point} if they
...
\max{this paragraph isn't really saying anything}
Regular expressions
...
\max{who cares what every discussion of regular expressions is accompanied by}
...
\newcommand{\epstransitions}{\varepsilon\mathsf{transitions}}
...
\newcommand{\epssrc}{\varepsilon\mathsf{src}}
\newcommand{\epsdst}{\varepsilon\mathsf{dst}}
...
\item $N.\labelt$ --- a mapping from $N.\transitions$ to the character labeling the transition
\item $N.\src$ and $N.\dst$ --- mappings from $N.\transitions$ to source and target states of a labeled transition
\item $N.\epstransitions$ --- a finite set of $\varepsilon$-transitions
\item $N.\epssrc$ and $N.\epsdst$ --- mappings from $N.\epstransitions$ to source and target states of an $\varepsilon$-transition
\end{itemize}
...
$\varepsilon$-transition then we may use $\epscons$ to pull it back along the
$\varepsilon$-transition and construct a trace beginning at the source of the $\varepsilon$-transition.
...
\item $D.\isAcc$ --- a decidable proposition over $D.\states$ denoting which
      states are accepting
\item $D.\delta$ --- a deterministic transition function. Given a character $c$ and a state
        $s$, $D.\delta_{c}(s)$ is the state for which there is a transition $s \overset{c}{\to} D.\delta_{c}(s)$
\end{itemize}

\begin{figure}
\begin{align*}
\data &~\Trace_{D} : (s : D.\states) (b : \Bool) \to L~\where\\
      & \nil : \ltonl {\left( \LinPiTyLimit {s} {D.\states} {\Trace_{D}(s , D.\isAcc(s))} \right)}  \\
      & \cons : \ltonl {\left( \LinPiTyLimit {c} {\Sigma} {\LinPiTyLimit {s} {D.\states} {c \lto \LinSigTyLimit {b}{\Bool}{\Trace_{D}~(D.\delta_{c}(s), b)} \lto \Trace_{D}(s, b)}} \right)}
\end{align*}
\caption{Traces through the DFA $D$ as an indexed inductive type}
\label{fig:dfatrace}
\end{figure}

\steven{TODO make the DFA parser figure fit}

The linear type of traces through $D$ is given in \cref{fig:dfatrace}. These
traces are either terminated in a state $s$ with the $\nil$ constructor. When
terminating a trace, we tag it with a Boolean that marks if the end state was
accepting or not. The $\cons$ constructor builds a trace out of state $s$ by linearly combining
a parse of some character $c$ with a
trace out of the state $D.\delta_{c}(s)$. The trace built with $\cons$ is
...
\inferrule
{
  \cdot \vdash mt := \withlamb {s} {\sigma~(D.\isAcc(b))~(\nil.\pi~s)}   : \LinPiTyLimit {s} {D.\states} {\LinSigTyLimit {b} {\Bool} {\Trace_{D}(s, b)}}
  \\
  ch : \charg , tr : \LinPiTyLimit {s} {D.\states} {\LinSigTyLimit {b} {\Bool } {\Trace_{D}(s, b)}} \vdash
  f := \withlamb {s} {\letin {\sigma~c~p_{c}} {ch} {\letin {\sigma~b~t} {tr(D.\delta_{c}(s))} {\sigma~b~(\cons(p_{c} , t))}}}
  : \LinPiTyLimit {s} {D.\states} {\LinSigTyLimit {b} {\Bool } {\Trace_{D}(s, b)}}
}
{w : \stringg \vdash \parse_{D} := \fold(mt, f)(w) : \LinPiTyLimit {s} {D.\states} {\LinSigTyLimit {b} {\Bool } {\Trace_{D}(s, b)}}}
\]
...
\max{I've seen people use a checkmark or a cute little agda logo for this}
...
\begin{proof}
  ...
  states of $D$ to be the $\mathbb{P}_{\varepsilon}(N.\mathsf{states})$ --- the type of $\varepsilon$-closed subsets of
  ...
  $\varepsilon$-closure of $N.\mathsf{init}$. Lastly define the transtion function
  of $D$ to send the subset $X$ under the character $c$ to the
  $\varepsilon$-closure of all the states reachable from $X$ via a transition
...
parametrized by $\varepsilon$-closed subsets containing $s'$.
. We instantiate this family at $\delta_{c}(X)$, as
$s'$ is reachable from $s$ by a single transition labelled by $c$, so
$s' \in \overline{\varepsilon}\overline{c}{X} = D.\delta_{c}(X)$. Therefore we can
...
Lastly, if the NFA trace was an $\varepsilon$-transition
$s \overset{\varepsilon}{\to} s'$ with $s \in X$, then the construction is quite similar. In this
...
states in $D$ are taken to be $\varepsilon$-closed, therefore $s' \in X$ because
$s$ is in $X$ and $\varepsilon$-transitions to $s'$.
...
some accepting $\varepsilon$-closed subset $X$, then we may choose some state
...
$\varepsilon$-transitions followed by a transition labeled by $c$.
We order all
...
\cite{thompsonProgrammingTechniquesRegular1968} in which we \max{I induct this sentence into the hall of shame for misuse of the word ``induct''}induct on the
...
grammars (CFGs). \max{In many ways?}In many ways, the smallest nontrivial CFG is the Dyck grammar
...
the empty string is balanced, and the $\balanced$ constrcutors builds a balanced
...
\data &~\Dyck : L~\where\\
      & \nil : \ltonl {\Dyck} \\
      & \balanced : \ltonl {(\stringquote{(} \lto \Dyck \lto \stringquote{)} \lto \Dyck \lto \Dyck)}
\end{align*}
\caption{Kleene Star as an inductive type}
...
We construct a parser for $\Dyck$ by building a determinstic automaton $M$ such
...
To justify our intuitive explanation of linear types as formal
grammars, we will now define a \emph{denotational semantics} that
makes this mathematically precise. This semantics is the basis for our
prototype implementation in Agda.

\subsection{What is a formal grammar?}
...
definition of a formal grammar is to use the Chomskyan formalism of
generative grammars where a grammar is given by a set of
non-terminals, with a specified start symbol and set of production
rules. We instead use a more abstract formulation that is closer in
spirit to the standard definition of a formal language:
...
To handle our universe, we assume a universe of small sets in our
metatheory that is closed under all of our type formers.
...
\begin{figure}
  \begin{align*}
    ...
    \sem{\el(A)}\gamma\,G &= \sem{A}\gamma\,G\\
    \sem{\mu A}\gamma
  \end{align*}
