...
manner by which \stringquote{ac} matches the grammar \fingram.
...
\begin{mathpar}
  ...
  {x : a , y : b \vdash \inl(x \otimes y) : \fingram}
\end{mathpar}
...
$M : \ntonl A^{*}$ denotes a linear term in the empty context
...
``$\nil$'' and ``$\cons$''
as symbols for contructors. That is, the $\nil$ constructor is not linearly
...
matches $(a^{*} \otimes b) \oplus c$ through repeated application of the Kleene star constructors.
...
\begin{figure}
\begin{align*}
\data &~\Trace : (s : N.\mathsf{state}) \to L~\where\\
