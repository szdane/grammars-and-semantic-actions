...
grammars and parses internal to it.

\paragraph{Finite Grammars}
First, consider finite grammars over the alphabet $\{ a , b , c \}$. Each letter of the
...
A finite grammar is built from base types via dijunctions $\oplus$ and
...
Next, we demonstrate how to parse a regular expression in \theoryabbv. Again,
consider grammars over $\{ a , b , c \}$. Similar to finite grammars, regular
...
$A^{*}$ in context comprising abstract\footnote{A linear type that is not a
  finite grammar.} linear types. In these contexts, it is not quite appropriate
...
term $A^{*}$ is a transformation of parse trees of the input grammar into parse
...
%% 1. First example: finite grammars (regex minus Kleene star)
%%    - why ordered linear:
%%      + no exchange
%%      + no contraction
%%      + no weakening
%%      + ambiguity
%% 2. Second example: Kleene star
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule
    {~}
    {\Gamma , x : \square A ; \cdot \vdash \bang x : A}
    %
    \and
    %
    \inferrule
    {\Gamma \vdash f : \square (A \lto B) \\
      \Gamma \vdash g : \square (A \lto B) \\
      \Gamma ; \Delta \vdash a : A \\
      \Gamma ; \Delta \vdash p : \apptol {\bang f}{a} = \apptol {\bang g}{a}}
    {\Gamma ; \Delta \vdash \equalizerpair{a}{p} : \equalizer {a}{\bang f}{\bang
      g}}
    %
    \and
    %
    ...
    {\Gamma \vdash f : \square (A \lto B) \\
      \Gamma \vdash g : \square (A \lto B) \\
      \Gamma ; \Delta \vdash e : \equalizer{a}{\bang f}{\bang g}}
...
%
    ...
    \inferrule
    {\Gamma \vdash A : L_i}
    {\Gamma \vdash \square A : U_i}
    %
    \and
    \inferrule
    {\Gamma ; \Delta \vdash f : A \\ \Gamma \vdash g : \square (A \lto B)}
    {\Gamma ; \Delta \vdash \equalizer {a}{\bang f}{\bang g} : L_i}
  \end{mathpar}
...
%
    ...
    {\Gamma \vdash f : \square (A \lto B) \\
      \Gamma \vdash g : \square (A \lto B) \\
      \Gamma ; \Delta \vdash a : A \\
      \Gamma ; \Delta \vdash p : \apptol {\bang f}{a} \equiv \apptol {\bang g}{a}}
...
\section{Formal Grammar Theory in Lambek}
