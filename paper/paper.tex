...
left implicit. For instance, every linear context is parametrized by a nonlinear
...
NFA parametrized by the first state in the trace. For the example NFA, the data
type $\Trace$ is provided in \cref{fig:nfainductive}.

$\Trace~s$ denotes traces through the NFA beginning in state $s$ and ending in
an accepting state. In this instance there is only a sinlge accepting state, and
thus the traces necessarily end in state 3. Let us dissect the structure
of $\Trace~s$ in more detail.

There are two kinds of constructors for a trace: those those modelling the
termination of a trace at a given state and those modelling transitions in the
...
Just as in \cref{fig:kleenestarderivation} shows the \stringquote{ab} matches
$(a^{*} \otimes b) \oplus c$, we show in TODO CREF that \stingquote{ab} is
accepted by the NFA representation of this regular expression. This is denote by
showing that \stringquote{ab} matches the trace grammar where the index is the
initial state of the NFA.\ Further, as we
will see in LATER SECTION, we can derive an isomorphism between parses of the
regular expression and accepting traces of the corresponding NFA.
