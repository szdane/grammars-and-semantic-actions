...
\begin{frame}{Parsing is Everywhere!}
  \talknote{A parser transforms user input into a machine-readable format}
  \talknote{Anytime we execute high-level PLs, we rely on a parser}
  \talknote{Parsers ought to be verified in all verified projects}
  \begin{center}
  \begin{tikzpicture}[
...
\talknote<5>{Completeness is not for free, but is easy to achieve in practice}
\onslide<5->{
...
\begin{frame}{Domain-Specific Language for Verified Parsers}
   Extends Joachim Lambek's \emph{Syntactic Calculus}
   \begin{itemize}
...
\begin{frame}{Why Linearity?}
  ...
  \begin{itemize}
    \item<2-> The context is an ordered list of resources
...
\item<+-> Disjunction $\oplus$, conjunction $\amp$
...
\talknote{This is how you configure arbitary CFGs. With productions as constructors}
\talknote{Dyck is either empty, or a parse of open paren, a Dyck parse, close
...
\talknote{boolean tags accepting/rejecting}
\talknote{nat marks which state the trace begins in}
\talknote{push/pop allow us to describe transitions. To construct a trace from the
src of a transition, parse the label and a trace from the destination}
\talknote{unexpected fails at 0 when trying to use an unwritten transition }
\talknote{This same strategy can encode the traces of other automata}
\begin{center}
\onslide<1-8>{
...
\begin{frame}{A Parser for the Dyck Grammar}
\talknote{Soundness guaranteed by the type system}
\talknote{Completeness guaranteed by the second iso}
  \onslide<1->{
...
\begin{alertblock}{Remember}
  Grammar is fundamentally ordered and linear.

  We may leverage this to design better tools for building parsers.
\end{alertblock}

Please take a look at
\href{https://github.com/maxsnew/grammars-and-semantic-actions}{our code} and at
\href{https://maxsnew.com/docs/lambek.pdf}{a preprint} of this work
\end{frame}
